...
export default {
  ...
  created() {
    this.knowledgeBullet.id = this.$route.query.id;
    this.getKnowledgeBullet();
    this.getSubjects();
  },
  methods: {
    getSubjects() {
    ...
    getKnowledgeBullet() {
      this.$axios.get("manage/knowledgeBullet/findById", {params: {id: this.knowledgeBullet.id}})
          .then(res => {
            this.knowledgeBullet = res.data.data;
            console.log("知识点JSON对象：" + this.knowledgeBullet);
          });
    },
    submitForm() {
      ...
      this.$refs.knowledgeBulletForm.validate((valid) => {
        if (valid) {
          ...
          this.$axios.post("manage/knowledgeBullet/update", this.knowledgeBullet)
              .then(res => {
